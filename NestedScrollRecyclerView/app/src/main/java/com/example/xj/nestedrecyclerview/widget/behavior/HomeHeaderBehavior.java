    ...
    public void onNestedPreScroll(CoordinatorLayout coordinatorLayout, View child, View target, int dx, int dy, int[] consumed, int type) {
        ...
        if (isUp) {
            ...
            if (isRecyclerViewBottom(child)) {
                ...
                Log.i(TAG, "--------->onNestedPreScroll: -->110");
            }
        } else {
            //非打开状态都要进行联滚
            if (!isOpen(child)) {
                ...
                Log.i(TAG, "--------->onNestedPreScroll: -->111");
            }
        }//else
    ...
    public void onNestedScroll(CoordinatorLayout coordinatorLayout, View child, View target,
        ...
        if (isUp) {
            ...
            if (isRecyclerViewBottom(child)) {
                ...
                onTranslationFinished(child);
                Log.i(TAG, "--------->onNestedScroll:* -->00");
            } else {
                Log.i(TAG, "--------->onNestedScroll:* -->01");
            }
        } else {
            if (!isOpen(child)) {
                child.setTranslationY(child.getTranslationY() + dyUnconsumed);
                onTranslationFinished(child);
                Log.i(TAG, "--------->onNestedScroll:* -->02");
            } else {
                Log.i(TAG, "--------->onNestedScroll:* -->03");
            }
        }
    }
