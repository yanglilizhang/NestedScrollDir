...
public class NestedScrollingParentLayout extends LinearLayout implements NestedScrollingParent {
    ...
    /**
     * 在嵌套滑动的子View未滑动之前，判断父view是否优先与子view处理
     * (也就是父view可以先消耗(滑动)，然后给子view消耗(滑动))
     *
     * @param target   具体嵌套滑动的那个子类RecylerView
     * @param dx       水平方向嵌套滑动的子View想要变化的距离
     * @param dy       垂直方向嵌套滑动的子View想要变化的距离 dy<0向下滑动 dy>0 向上滑动
     * @param consumed 这个参数要我们在实现这个函数的时候指定，回头告诉子View当前父View消耗的距离
     *                 consumed[0] 水平消耗的距离，consumed[1] 垂直消耗的距离 好让子view做出相应的调整
     *
     * 判断子View是否滑动到底部， recyclerView.canScrollVertically(1);返回false表示不能往上滑动，即代表到底部了；
     * 判断子View是否滑动到顶部， recyclerView.canScrollVertically(-1);返回false表示不能往下滑动，即代表到顶部了；
     */
    @Override
    public void onNestedPreScroll(@NonNull View target, int dx, int dy, @NonNull int[] consumed) {
        //  View.getScrollX() = 0 - x;/View.getScrollY() = 0 - y;
        Log.e("**", "----------->想要变化的距离:dy=" + dy + ",父滑动总:getScrollY()=" + getScrollY() + ",mTopViewHeight=" + mTopViewHeight);
        boolean hideTop = dy > 0 && getScrollY() < mTopViewHeight;//550 < mTopViewHeight
        boolean showTop = dy < 0 && getScrollY() >= 0 && !target.canScrollVertically(-1);
        Log.e("**", "----------->hideTop=" + hideTop + ",showTop=" + showTop);
        if (hideTop || showTop) {
            //父消耗滑动
            scrollBy(0, dy);
            //告诉子View被父消耗的距离，无这句子View也会滑动
            consumed[1] = dy;
        }
    }
